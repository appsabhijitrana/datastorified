...
export function DecisionRelatedTools({ workflow }: { workflow: DecisionWorkflow }) {
  return <div className="grid min-w-0 gap-4 sm:grid-cols-2"><Card className="min-w-0 p-5"><div className="flex items-center gap-2"><Calculator className="text-primary" /><h2 className="font-bold">Related calculators</h2></div><div className="mt-4 space-y-2">{workflow.relatedCalculators?.map((slug) => <Link key={slug} href={`/calculators/${slug}`} className="flex min-w-0 items-center justify-between gap-3 rounded-xl bg-soft px-3 py-3 text-sm font-semibold hover:text-primary"><span className="truncate">{title(slug)}</span><ArrowUpRight className="shrink-0" size={15} /></Link>)}</div></Card><Card className="min-w-0 p-5"><div className="flex items-center gap-2"><Wrench className="text-accent" /><h2 className="font-bold">Useful tools</h2></div><div className="mt-4 space-y-2">{workflow.relatedTools?.map((slug) => <Link key={slug} href={`/tools/${slug}`} className="flex min-w-0 items-center justify-between gap-3 rounded-xl bg-soft px-3 py-3 text-sm font-semibold hover:text-primary"><span className="truncate">{title(slug)}</span><ArrowUpRight className="shrink-0" size={15} /></Link>)}</div></Card></div>;
}
