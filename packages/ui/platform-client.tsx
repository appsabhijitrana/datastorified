...
const consentKey = "ds.analytics.consent";
...
export function PlatformClient() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setAnalyticsConsent(stored === null ? null : stored === "true");
    } catch {
  ...
  const setConsent = (value: boolean) => {
    ...
    try { window.localStorage.setItem(consentKey, String(value)); } catch { /* Ignore storage errors in private mode. */ }
  };
  return <>
    ...
    {analyticsConsent === null && <Card className="fixed bottom-24 left-4 right-4 z-[95] mx-auto max-w-xl p-4 shadow-lift md:bottom-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Help improve DataStorified</p><p className="mt-1 text-sm text-muted">Allow anonymous analytics to understand product usage and errors.</p></div><div className="flex gap-2"><Button onClick={() => setConsent(true)}>Allow</Button><Button variant="ghost" onClick={() => setConsent(false)}>No thanks</Button></div></div></Card>}
...
export function AnalyticsScripts() {
  ...
  const consentGranted = typeof window !== "undefined" && window.localStorage.getItem(consentKey) === "true";
  return <>{gaId && consentGranted && <><Script src={`https://www.googletagmanager.com/gtag/js?id=${gaId}`} strategy="afterInteractive"/><Script id="ga4" strategy="afterInteractive">{`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}window.gtag=gtag;gtag('js',new Date());gtag('config',${JSON.stringify(gaId)},{anonymize_ip:true});`}</Script></>}</>;
}
