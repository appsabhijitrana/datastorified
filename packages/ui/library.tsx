...
const LazyScenarioProjectionChart = lazy(() => import("./recharts-charts").then((module) => ({ default: module.ScenarioProjectionChartInner })));
const LazyBarComparisonChart = lazy(() => import("./recharts-charts").then((module) => ({ default: module.BarComparisonChartInner })));
...
export function ScenarioProjectionChart({ data, title = "Scenario projection", description }: { data: Array<{ label: string; value: number }>; title?: string; description?: string }) {
  return (
    <Suspense fallback={<Card className="p-5"><div className="h-56 animate-pulse rounded-3xl bg-soft" /></Card>}>
      <LazyScenarioProjectionChart data={data} title={title} description={description} />
    </Suspense>
  );
}
...
export function BarComparisonChart({ title, leftLabel, rightLabel, leftValue, rightValue }: { title?: string; leftLabel: string; rightLabel: string; leftValue: number; rightValue: number }) {
  return (
    <Suspense fallback={<Card className="p-5"><div className="h-56 animate-pulse rounded-3xl bg-soft" /></Card>}>
      <LazyBarComparisonChart title={title} leftLabel={leftLabel} rightLabel={rightLabel} leftValue={leftValue} rightValue={rightValue} />
    </Suspense>
  );
}
