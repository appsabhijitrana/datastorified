...
import type { DecisionRecommendation as Recommendation, DecisionRecommendationResult } from "@datastorified/decision-os";
import type { ProfileAnalysis } from "@datastorified/profile";

type RecommendationLike = Pick<Recommendation, "title" | "summary"> | DecisionRecommendationResult;

export function DecisionRecommendation({ recommendation, analysis, note }: { recommendation: RecommendationLike; analysis?: Pick<ProfileAnalysis, "label" | "description" | "nextBestField" | "percentage">; note?: string }) {
  const title = "title" in recommendation ? recommendation.title : recommendation.summary;
    ...
    <Card className="min-w-0 border-primary/15 bg-gradient-to-br from-primary/[.06] to-accent/[.08] p-6">
      <div className="flex gap-3">
        ...
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-balance text-2xl font-bold">{title}</h2>
        </div>
      </div>
